...
class actor_network(nn.Module):
    def __init__(self, config):
        ...
        self.pi_backbone = nn.Sequential(
            nn.Linear((int(self.config["node_feature_num"]) * self.config["node_num"] + self.config["queue_feature_num"] + self.config["link_num"]) * 2, self.config["hidden_feature_num"]),
            nn.ReLU(),
            nn.Linear(self.config["hidden_feature_num"], self.config["hidden_feature_num"]), 
            nn.ReLU(),
            nn.Linear(self.config["hidden_feature_num"], self.config["hidden_feature_num"]), 
    ...
    def gnn(self, state):
        ...
        # =========================================================================
        #node_feature_1 = self.pi_s_ecc1(node_feature_1, adjacency_1, link_feature_1)
        #node_feature_1 = self.pi_graph_u_net1(node_feature_1, adjacency_1)
        ...
        concat_1 = torch.cat([readout_1/self.config["state_weight"], job_waiting_feature_1, link_feature_1.view(data_num_1, -1)], dim=1) # 여기에 job waiting 벡터 붙이기.

        # =========================================================================
        #node_feature_2 = self.pi_s_ecc1(node_feature_2, adjacency_2, link_feature_2)
        #node_feature_2 = self.pi_graph_u_net1(node_feature_2, adjacency_2)
        ...
        concat_2 = torch.cat([readout_2/self.config["state_weight"], job_waiting_feature_2, link_feature_2.view(data_num_1, -1)], dim=1) # 여기에 job waiting 벡터 붙이기.
